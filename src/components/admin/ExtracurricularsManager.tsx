...
export const ExtracurricularsManager = ({ initialData = [], onUpdate }: ExtracurricularsFormProps) => {
    const [items, setItems] = useState(initialData);
    const [loading, setLoading] = useState(false);

    const handleAdd = () => {
        setItems([...items, { title: '', category: 'Volunteering', image_url: '', description: '' }]);
    };
    ...
    const handleSave = async (index: number) => {
        ...
        let result;
        if (item.id) {
            result = await supabase.from('extracurriculars').update(item).eq('id', item.id);
        } else {
            result = await supabase.from('extracurriculars').insert(item);
        }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-[var(--color-jet-black)]">Manage Extracurriculars</h3>
                <button
                    onClick={handleAdd}
            ...
            {items.map((item, idx) => (
                <GlassCard key={idx} className="relative">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            <label className="mb-1 block text-xs font-bold uppercase text-[var(--color-deep-slate)]">Category</label>
                            <select
                                value={item.category}
                                onChange={(e) => {
                                    const newItems = [...items];
                        ...
                        <div>
                            <label className="mb-1 block text-xs font-bold uppercase text-[var(--color-deep-slate)]">Image URL (Optional)</label>
                            <input
